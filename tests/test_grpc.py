...
async def main():
    ...
    resp = await stub.ProcessMaterial(
        knowledge_pb2.ProcessMaterialRequest(
    ...
    # 2. StoreTopics
    print("\n2. StoreTopics...")
    resp = await stub.StoreTopics(
        knowledge_pb2.StoreTopicsRequest(
            workflow_id="wf_grpc_test",
            topics=[
    ...
    # 3. GetTopics
    print("\n3. GetTopics...")
    resp = await stub.GetTopics(
        knowledge_pb2.GetTopicsRequest(
            workflow_id="wf_grpc_test",
        )
    )
    ...
    resp = await stub.SimilaritySearch(
        knowledge_pb2.SimilaritySearchRequest(
    ...
    # 5. SearchPolicies
    print("\n5. SearchPolicies...")
    resp = await stub.SearchPolicies(
        knowledge_pb2.SearchPoliciesRequest(
    ...
    print("\n6. GetChunksByWorkflow...")
    resp = await stub.GetChunksByWorkflow(
        knowledge_pb2.GetChunksByWorkflowRequest(
            workflow_id="wf_grpc_test",
        )
    )
    ...
    print("\n7. GetChunksByIds...")
    if chunk_ids:
        resp = await stub.GetChunksByIds(
            knowledge_pb2.GetChunksByIdsRequest(
